...
当前白名单包含 `toast`、`statusBar`、`haptic`、`saveImage`、`share`、`backToHome`、`exitApp` 和 `checkUpdate`。参数结构以 `frontend/src/sdk/nativeApp.ts` 为准；Android 会再次校验方法、来源和参数，不提供任意原生命令入口。
