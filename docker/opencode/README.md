...
OpenCode 内置的 Exa 搜索通过 `OPENCODE_ENABLE_EXA=1` 启用。用户可在 Marvo 智能体设置中保存自己的 Exa API Key；API Key 加密后存放在未挂载进 Agent 的 `app/` 目录中，只在创建对应用户容器时作为 `EXA_API_KEY` 注入。API Key 变更后，该用户的旧容器会在处理下一次请求前重建。
